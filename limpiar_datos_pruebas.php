...
if ($ejecutar) {
    try {
        ...
        // Auto-crear tabla expedientes_autorizaciones_cc solo si no existe en la base de datos
        $chkAut = $pdo->query("SHOW TABLES LIKE 'expedientes_autorizaciones_cc'")->fetchColumn();
        if (!$chkAut) {
            $pdo->exec("
        ...
        foreach ($tablas as $t) {
            ...
            if ($chkTable) {
                ...
                try {
                    ...
                    // Si el usuario MySQL carece del privilegio ALTER, la tabla queda vacía igualmente
                }
                $detalles[$t] = $cnt;
            }
        }
