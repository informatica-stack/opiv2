...
<body class="bg-slate-50 text-slate-800 font-sans d-flex flex-column min-vh-100">
            ...
            <div class="card shadow-sm border-light mb-4">
                ...
                <div class="card-body p-4">
                    <div class="row g-3">
                        ...
                        <!-- ALERTA DE TOPE DE COMPRA ÁGIL (Art. 10 bis Ley 19.886) -->
                        <div class="col-12 d-none" id="alertaTopeCompraAgil">
                            <div class="alert alert-danger d-flex align-items-start gap-3 p-3 rounded-3 shadow-sm border-danger mb-0" role="alert">
                                <i class="bi bi-exclamation-octagon-fill fs-3 text-danger flex-shrink-0 mt-1"></i>
                                <div>
                                    <h6 class="fw-bold mb-1 text-danger">⚠️ Límite Excedido para Compra Ágil (Art. 10 bis Ley 19.886)</h6>
                                    <div id="textoAlertaCompraAgil" class="small mb-1 text-danger-emphasis">
                                        El monto ingresado excede el tope legal de <strong>100 UTM</strong> ($<?= number_format(100 * VALOR_UTM, 0, ',', '.') ?> CLP).
                                    </div>
                                    <div class="small fw-semibold text-danger">
                                        <i class="bi bi-arrow-right-circle me-1"></i>Para continuar, reduzca el monto de cotización o cambie la Modalidad / Tipo de Compra a <strong>Licitación Pública</strong>.
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        const rangosUtmData = <?= json_encode($rangos_utm) ?>;
        const valorUtmVigente = <?= VALOR_UTM ?>;
        let excedeCompraAgilGlobal = false;
        let montoActualUtmGlobal = 0.0;
        ...
        function calc() {
            ...
            const montoEvaluarUtm = totalBruto;
            ...
            if (Array.isArray(rangosUtmData)) {
                for (let i = 0; i < rangosUtmData.length; i++) {
                    const r = rangosUtmData[i];
                    const min = parseFloat(r.min_utm) || 0;
                    const max = (r.max_utm !== null && r.max_utm !== undefined && r.max_utm !== '') ? parseFloat(r.max_utm) : 999999999;
                    if (montoActualUtmGlobal >= min && montoActualUtmGlobal <= max) {
                        rangoAsignado = r;
                        break;
                    }
                }
                if (!rangoAsignado && rangosUtmData.length > 0) {
                    rangoAsignado = rangosUtmData[rangosUtmData.length - 1];
                }
            }

            const selRango = document.getElementById('selRangoUtm');
            ...
            const selectTC = document.getElementById('selTipoCompra');
            const tcId = selectTC ? selectTC.value : '';
            const tcCodigo = mapaTiposCompra[tcId] || '';
            const esCompraAgil = (tcCodigo === 'COMPRA_AGIL' || tcCodigo === 'AGIL');
            const topeAgilUtm = 100.00;
            const alertaAgil = document.getElementById('alertaTopeCompraAgil');
            const txtAlerta = document.getElementById('textoAlertaCompraAgil');
            const inpMontoCot = document.getElementById('inpMontoDisponible');

            excedeCompraAgilGlobal = (esCompraAgil && montoActualUtmGlobal > topeAgilUtm);

            if (alertaAgil) {
                if (excedeCompraAgilGlobal) {
                    alertaAgil.classList.remove('d-none');
                    const topePesos = formatter.format(Math.round(topeAgilUtm * valorUtmVigente));
                    if (txtAlerta) {
                        txtAlerta.innerHTML = `El monto total estimado de <strong>${formatter.format(montoEvaluarUtm)} CLP (${montoActualUtmGlobal.toFixed(2)} UTM)</strong> excede el tope legal de <strong>100 UTM (${topePesos})</strong> para Compra Ágil (Art. 10 bis Ley 19.886).`;
                    }
                    if (inpMontoCot) inpMontoCot.classList.add('is-invalid', 'border-danger');
                } else {
                    alertaAgil.classList.add('d-none');
                    if (inpMontoCot) inpMontoCot.classList.remove('is-invalid', 'border-danger');
                }
            }
            ...
            if (btnSub) {
                if (excedeCompraAgilGlobal) {
                    ...
                    btnSub.title = 'Monto excede 100 UTM para Compra Ágil';
                } else {
        ...
        function validarFormulario() {
            ...
            if (excedeCompraAgilGlobal) {
                divError.innerHTML = "<strong>⛔ Tope Excedido:</strong> El monto estimado supera el límite máximo de 100 UTM permitido para Compra Ágil. Seleccione Licitación Pública o reduzca el monto.";
                divError.classList.remove('d-none');
                alert('No es posible continuar:\n\nEl monto supera el tope legal de 100 UTM para Compra Ágil.\nReduzca el monto o seleccione Licitación Pública.');
                return false;
            }
            ...
            if (errorItems) {
                divError.innerHTML = "<strong>⚠️ Faltan datos obligatorios:</strong> Por favor complete todos los campos obligatorios del detalle de productos (en Convenio Marco, el ID Convenio Marco debe ser únicamente numérico).";
                divError.classList.remove('d-none');
