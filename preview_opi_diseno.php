...
// Columna Izquierda: Institucion y Unidad
$pdf->SetXY(38, 8);
$pdf->SetFont('Arial', '', 7.5);
$pdf->Cell(60, 3.5, $utf("Republica de Chile"), 0, 1, 'L');
$pdf->SetX(38);
$pdf->SetFont('Arial', 'B', 8.5);
$pdf->Cell(60, 4, $utf("MUNICIPALIDAD DE LEBU"), 0, 1, 'L');
$pdf->SetX(38);
$pdf->SetFont('Arial', 'B', 7.5);
$pdf->Cell(60, 3.5, $utf("DEPARTAMENTO DE ADQUISICIONES"), 0, 1, 'L');

// Columna Centro: Titulo Oficial Configurable
$pdf->SetXY(98, 10);
$pdf->SetFont('Arial', 'B', 11);
$pdf->Cell(60, 5, $utf($titulo_doc), 0, 1, 'C');

// Columna Derecha: Folio Muestra y Fecha
$pdf->SetXY(158, 8);
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(44, 4.5, $utf("N° OPI-" . date('Y') . "-0001 (VISTA PREVIA)"), 0, 1, 'R');
$pdf->SetXY(148, 13);
$pdf->SetFont('Arial', '', 8);
$pdf->Cell(54, 4, $utf("Lebu, " . pdf_fecha_espanol(date('Y-m-d'))), 0, 1, 'R');
...
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(180, 4, $utf("DIRECCION DE DESARROLLO COMUNITARIO (DIDECO)"), 0, 1, 'L');

$pdf->SetX(14);
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(8, 4, $utf("A:"), 0, 0, 'L');
$pdf->SetFont('Arial', '', 8);
$pdf->Cell(180, 4, $utf("DIRECCION DE ADMINISTRACION Y FINANZAS - UNIDAD DE ADQUISICIONES"), 0, 1, 'L');

// --- CLAUSULA 1: TEXTO INTRODUCTORIO CONFIGURABLE ---
$pdf->SetXY(14, 34);
$pdf->SetFont('Arial', '', 7.5);
$pdf->Cell(188, 4, $utf($clausula1_txt), 0, 1, 'L');
...
$pdf->Cell(96, 5, $utf("DESCRIPCION DE LOS BIENES Y/O SERVICIOS"), 1, 0, 'L', true);
$pdf->Cell(30, 5, $utf("MONTO UNITARIO"), 1, 0, 'R', true);
$pdf->Cell(30, 5, $utf("MONTO TOTAL"), 1, 1, 'R', true);

$pdf->SetFont('Arial', '', 7.5);
$items_muestra = [
    ['cant' => 10, 'unidad' => 'UNID', 'desc' => 'RESMAS DE PAPEL CARTA 75 GR MULTIPROPOSITO (ID CM: 154238)', 'unit' => 4500, 'total' => 45000],
    ['cant' => 5,  'unidad' => 'GLOBAL', 'desc' => 'SERVICIO DE MANTENCION PREVENTIVA DE EQUIPOS COMPUTACIONALES', 'unit' => 60000, 'total' => 300000],
    ['cant' => 2,  'unidad' => 'SET', 'desc' => 'PACK DE TONER NEGRO DE ALTO RENDIMIENTO PARA IMPRESORAS', 'unit' => 35000, 'total' => 70000],
    ['cant' => '', 'unidad' => '', 'desc' => '', 'unit' => '', 'total' => '']
];

foreach ($items_muestra as $it) {
    $pdf->SetX(14);
    $pdf->Cell(14, 4.8, $it['cant'] ? number_format($it['cant'], 0, ',', '.') : '', 1, 0, 'C');
    $pdf->Cell(18, 4.8, $utf($it['unidad']), 1, 0, 'C');
    $pdf->Cell(96, 4.8, $utf(" " . $it['desc']), 1, 0, 'L');
    $pdf->Cell(30, 4.8, $it['unit'] ? "$ " . number_format($it['unit'], 0, ',', '.') : '', 1, 0, 'R');
    $pdf->Cell(30, 4.8, $it['total'] ? "$ " . number_format($it['total'], 0, ',', '.') : '', 1, 1, 'R');
}
...
$pdf->SetFont('Arial', '', 7.5);
$pdf->Cell(30, 4.5, "$ " . number_format($total_neto, 0, ',', '.'), 1, 1, 'R');
...
$pdf->SetFont('Arial', '', 7.5);
$pdf->Cell(30, 4.5, "$ " . number_format($iva_calc, 0, ',', '.'), 1, 1, 'R');
...
$pdf->SetFont('Arial', 'B', 8.5);
$pdf->Cell(30, 5, "$ " . number_format($total_bruto, 0, ',', '.'), 1, 1, 'R');

// --- RECUADROS INSTITUCIONALES DE INFORMACION ---
$y_cajas = $pdf->GetY() + 3;
...
$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(25, 4, $utf("RAZON SOCIAL:"), 0, 0, 'L');
$pdf->SetFont('Arial', '', 7);
$pdf->Cell(120, 4, $utf("DISTRIBUIDORA Y SERVICIOS INTEGRALES SUR LTDA."), 0, 1, 'L');

$pdf->SetXY(54, $y_cajas + 6);
$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(12, 4, $utf("RUT:"), 0, 0, 'L');
$pdf->SetFont('Arial', '', 7);
$pdf->Cell(45, 4, $utf("76.890.123-4"), 0, 0, 'L');

$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(20, 4, $utf("DIRECCION:"), 0, 0, 'L');
$pdf->SetFont('Arial', '', 7);
$pdf->Cell(55, 4, $utf("Av. Ignacio Carrera Pinto N° 450, Lebu"), 0, 1, 'L');

// 2. RECUADRO IMPUTACION PRESUPUESTARIA
$y_caja2 = $y_cajas + 14;
...
$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(38, 4, $utf("IMPUTACION PRESUPUESTARIA:"), 0, 0, 'L');

$pdf->Cell(22, 4, $utf("CUENTA N°:"), 0, 0, 'L');
$pdf->SetFont('Arial', '', 7);
$pdf->Cell(40, 4, $utf("215.22.04.001"), 0, 0, 'L');

$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(26, 4, $utf("AREA GESTION:"), 0, 0, 'L');
$pdf->SetFont('Arial', '', 7);
$pdf->Cell(15, 4, $utf("01"), 0, 0, 'L');

$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(24, 4, $utf("CENTRO COSTO:"), 0, 0, 'L');
$pdf->SetFont('Arial', '', 7);
$pdf->Cell(22, 4, $utf("102 - DIDECO"), 0, 1, 'L');

$pdf->SetXY(54, $y_caja2 + 6);
$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(32, 4, $utf("COMPLEMENTARIA:"), 0, 0, 'L');
$pdf->SetFont('Arial', '', 6.5);
$pdf->Cell(95, 4, $utf("AUT. CC EXT: Direccion de Obras ($ 150.000 - APROBADO)"), 0, 1, 'L');
...
$pdf->SetFont('Arial', '', 7);
$pdf->Cell(50, 4, $utf("FORTALECIMIENTO GESTION COMUNITARIA"), 0, 0, 'L');
$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(15, 4, $utf("ITEM:"), 0, 0, 'L');
$pdf->SetFont('Arial', '', 7);
$pdf->Cell(45, 4, $utf("MATERIALES E INSUMOS OFICINA"), 0, 1, 'L');

$pdf->SetXY(16, $y_caja3 + 6.5);
$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(38, 4, $utf("C. SUMINISTROS ID:"), 0, 0, 'L');
$pdf->SetFont('Arial', '', 7);
$pdf->Cell(50, 4, $utf("CS-2026-089"), 0, 0, 'L');
$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(30, 4, $utf("CONV. MARCO O°C:"), 0, 0, 'L');
$pdf->SetFont('Arial', '', 7);
$pdf->Cell(45, 4, $utf("2345-789-CM26"), 0, 1, 'L');

$pdf->SetXY(16, $y_caja3 + 11.5);
$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(38, 4, $utf("COMPRA AGIL ID:"), 0, 0, 'L');
$pdf->SetFont('Arial', '', 7);
$pdf->Cell(50, 4, $utf("CA-8902"), 0, 0, 'L');
$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(34, 4, $utf("DECRETO ALCALDICIO N°:"), 0, 0, 'L');
$pdf->SetFont('Arial', '', 7);
$pdf->Cell(40, 4, $utf("DEC-ALC-758/2026"), 0, 1, 'L');

// --- CLAUSULA 2: DESTINO DE LOS BIENES / SERVICIOS ---
...
$motivo_ejemplo = "Insumos y servicios requeridos para el normal desarrollo de los talleres comunitarios vecinales y operatividad de la oficina de atencion ciudadana del Departamento durante el periodo 2026.";
$pdf->MultiCell(188, 3.5, $utf($motivo_ejemplo), 1, 'L');

// --- LINEAS DE BASE PARA LAS 3 FIRMAS DIGITALES (FIRMAGOB) ---
$y_firmas_line = $firmas_linea_y;
$pdf->SetDrawColor(120, 120, 120);
$pdf->SetLineWidth(0.3);

// Linea 1: Jefatura Unidad Solicitante (Izquierda: X=14 a 72, Ancho=58mm)
$pdf->Line(14, $y_firmas_line, 72, $y_firmas_line);

// Linea 2: Direccion de Finanzas / Presupuesto (Centro: X=79 a 137, Ancho=58mm)
$pdf->Line(79, $y_firmas_line, 137, $y_firmas_line);

// Linea 3: Administrador Municipal (Derecha: X=144 a 202, Ancho=58mm)
$pdf->Line(144, $y_firmas_line, 202, $y_firmas_line);

// --- SIMULACION DE ESTAMPAS DIGITALES FIRMAGOB (SI ESTA ACTIVA LA OPCION) ---
if ($simular_estampas === 1) {
    $dibujar_estampa_simulada = function($x, $y_base, $titulo_rol, $nombre, $run, $cargo) use ($pdf, $utf) {
        ...
        $pdf->Cell($w - 2, 3, $utf("FIRMADO ELECTRONICAMENTE (FEA)"), 0, 1, 'L');
        ...
        $pdf->Cell($w - 2.4, 2.2, $utf("Validez Legal: Ley N° 19.799 sobre Firma Electronica"), 0, 1, 'L');

        $pdf->SetTextColor(0, 0, 0);
    };

    // Estampa 1: Jefatura
    $dibujar_estampa_simulada(14, $y_firmas_line, "V°B° TECNICO JEFATURA", "JUAN CARLOS ARRIAGADA", "17.439.829-1", "Jefe DIDECO");
    // Estampa 2: Presupuesto
    $dibujar_estampa_simulada(79, $y_firmas_line, "V°B° PRESUPUESTARIO", "MARIA JOSE CONTRERAS", "15.987.654-3", "Encargada Presupuesto");
    // Estampa 3: Administrador
    $dibujar_estampa_simulada(144, $y_firmas_line, "AUTORIZACION FINAL", "ROBERTO SANDOVAL MORA", "12.345.678-9", "Administrador Municipal");
}
