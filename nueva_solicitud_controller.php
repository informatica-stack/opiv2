    ...
    try {
        ...
        if ($requiere_cot) {
            ...
            if ($post_tipo_impuesto === 'NETO') {
                $subtotal_neto = $monto_disp;
                $total_est_bruto = round($monto_disp * $iva_pct);
            } elseif ($post_tipo_impuesto === 'EXENTO') {
                $subtotal_neto = $monto_disp;
                $total_est_bruto = round($monto_disp);
            } else {
                $total_est_bruto = round($monto_disp);
                $subtotal_neto = round($total_est_bruto / $iva_pct);
            }
        } else {
            foreach ($cant as $i => $c) {
                ...
                $c_val = floatval($c);
                if ($post_tipo_impuesto === 'NETO') {
                    $subtotal_neto += ($c_val * $p_ingresado);
                    ...
                    $p_unit = round($p_ingresado, 2);
                    $subtotal_neto += round(($c_val * $p_unit) / $iva_pct);
                }
                $total_est_bruto += ($c_val * $p_unit);
            }
        }
        ...
        if (in_array($codigo_tc, ['AGIL', 'COMPRA_AGIL'])) {
        ...
        // ASIGNACIÓN AUTOMÁTICA DEL RANGO DE MONTO ESTIMADO (Art. 10)
        $rango_calculado = calcular_rango_utm_automatico($pdo, $monto_evaluar_utm, VALOR_UTM);
        if (!empty($rango_calculado['rango_id'])) {
            $post_rango_utm = $rango_calculado['rango_id'];
        }
